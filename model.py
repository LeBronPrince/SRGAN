...
def SRGAN_g(t_image, is_train=False, reuse=False):# reuse-false:create new variables. reuse-true:use exiting variables.
    ...
    with tf.variable_scope("SRGAN_g", reuse=reuse) as vs:
        ...
        # B residual blocks
        nn = Conv2d(n, 64, (3, 3), (1, 1), act=None, padding='SAME', W_init=w_init, b_init=b_init, name='n64s1/c1/0')
        nn = BatchNormLayer(nn, act=tf.nn.relu, is_train=is_train, gamma_init=g_init, name='n64s1/b1/0' )
        nn = Conv2d(nn, 64, (3, 3), (1, 1), act=None, padding='SAME', W_init=w_init, b_init=b_init, name='n64s1/c2/0')
        nn = BatchNormLayer(nn, is_train=is_train, gamma_init=g_init, name='n64s1/b2/0' )
        nn = ElementwiseLayer([n, nn], tf.add, 'b_residual_add/0')
        n = nn
...
def SRGAN_d(input_images, is_train=True, reuse=False):
    ...
    with tf.variable_scope("SRGAN_d", reuse=reuse):
        ...
        net_ho = FlattenLayer(net_h8, name='ho/flatten')# it a layer that reshape a high-dimension input to a vecter and then apply to DenseLayer  RNNlayer
        net_ho = DenseLayer(net_ho, n_units=1, act=tf.identity,   #[batch_size, mask_row, mask_col, n_mask] ---> [batch_size, mask_row * mask_col * n_mask]
